...
from forms import form_4_usersession as fg
...
def callback_existinguserform(username, credential):
    ...
    if checkusername:
        ...
        if lengthdata >0:
            st.session_state.geolocation_complete = fg.get_geolocation()
            st.session_state.usersession_complete = fg.create_user_session()
